...
/*   Updated: 2022/11/18 10:22:23 by yachaab          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	static char		*n_tr_line[OPEN_MAX];
	char			*tr_line;

	if (fd < 0 || BUFFER_SIZE <= 0 || fd > OPEN_MAX)
		return (NULL);
	if (!n_tr_line[fd])
		n_tr_line[fd] = ft_strdup("");
	n_tr_line[fd] = read_line(fd, n_tr_line[fd]);
	tr_line = trim(n_tr_line[fd]);
	n_tr_line[fd] = rest_line(n_tr_line[fd]);
	return (tr_line);
}
